...
/**
 * Same lookup for the aggregate endpoints, which send a stage as a bare
 * KEY (a `{ stage_key: count }` map) with no English label alongside it.
 * Unknown key → the raw key, for the same reason stageLabelTh() falls
 * back to the API's label: a stage added server-side must still render.
 */
export function stageLabelThForKey(key: string): string {
  return PIPELINE_STAGE_LABELS_TH[key] ?? key
}

/** One bucket of a `deals_by_stage` map, ready to render. */
...
/**
 * TASK-179 §4.1 (F-4, BR-7) — the ONE way this app turns a server-sent
 * `deals_by_stage` map into something renderable.
 *
 * `AgentDashboardMetricsService::dealsByStage()` and
 * `AgentSalesAggregateService::stageKeys()` both emit EVERY
 * `PipelineStage` case, in the enum's own declaration order, with a 0 for
 * stages nobody is sitting at. Since ADR-026 that is eight stages, not
 * five, and the set is a business value that changes with an ADR (BR-7).
 *
 * So this helper reads the map's OWN keys in the map's OWN order and
 * never names one. Three screens used to carry a hardcoded five-element
 * list instead — the funnel bars then summed to less than the ดีลทั้งหมด
 * KPI beside them, because the three post-sale stages were silently
 * dropped. A fixed-length array, a five-element interface or a
 * `s.complete_registered` field access re-creates that defect exactly.
 *
 * (Object key order is insertion order for non-integer-like string keys —
 * every stage key is snake_case — so JSON object order survives the trip.)
 */
export function stageCounts(dealsByStage: Record<string, number>): StageCount[] {
  return Object.entries(dealsByStage).map(([key, count]) => ({
    key,
    label: stageLabelThForKey(key),
    count,
  }))
}
