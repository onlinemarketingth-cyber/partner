...
/**
 * LoadingSkeleton — ported verbatim from frontend/src/design-system for
 * Admin parity (CLAUDE.md Section 7: design-system components are
 * duplicated between the two frontends, not shared via a package yet —
 * keep both copies in sync, CI-001/CI-002). Previously frontend-admin
 * had no loading-state component at all; every admin view showed a
 * blank page during its initial fetch.
 *
 * แสดง shimmer placeholder ระหว่างที่ Index/Detail page กำลัง fetch data
 * แทน blank screen (ที่ user เข้าใจผิดว่า app broken)
 *
 * Types:
 *   - list      : HeroHeader placeholder + N skeleton rows
...
<template>
    <div class="skeleton-wrap px-4 lg:px-6 pb-4 lg:pb-6 w-full"
        ...
        <div class="sk-hero rounded-2xl border border-slate-200 bg-white/95 shadow-sm p-5 mb-4">
        ...
        <template v-if="type === 'list'">
            <div class="space-y-2">
                <div v-for="i in rows" :key="'row-'+i"
                     class="sk-row rounded-xl border border-slate-200 bg-white/95 p-4 flex items-center gap-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div class="lg:col-span-2 space-y-4">
                    <div v-for="i in 3" :key="'d-'+i"
                         class="rounded-xl border border-slate-200 bg-white/95 p-5 space-y-3">
        ...
        <template v-else-if="type === 'dashboard'">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                <div v-for="i in 4" :key="'kd-'+i"
                     class="rounded-xl border border-slate-200 bg-white/95 p-4 space-y-2">
                    <div class="sk-shimmer sk-line sk-line-sm" style="width: 50%"></div>
                    <div class="sk-shimmer sk-line sk-line-kpi" style="width: 70%"></div>
                </div>
            </div>
            <div class="rounded-xl border border-slate-200 bg-white/95 p-5">
