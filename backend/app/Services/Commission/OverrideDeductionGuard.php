...
class OverrideDeductionGuard
{
    ...
    public function refusalFor(Company $company, CommissionRateType $rateType, int $rateValue, ?CommissionOverrideMode $assumingMode = null): ?string
    {
        ...
        foreach ($this->sellableProducts($company) as $product) {
            ...
            return sprintf(
                'อัตรานี้หักเกินค่าคอมของผู้ขาย — "%s" จ่ายตัวแทน %s แต่สายงานลึกสุด %d ชั้น '
                .'จึงหักได้ไม่เกินชั้นละ %s (ตอนนี้ตั้งไว้ชั้นละ %s) · '
                .'แก้ได้ 3 ทาง: ลดอัตราหัวหน้าทีม, เพิ่มอัตราตัวแทนในขั้นที่ 3, หรือเปลี่ยนเป็น "บริษัทจ่ายเพิ่ม"',
