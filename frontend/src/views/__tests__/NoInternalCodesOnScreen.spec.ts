 ...
 * (AgentCopyIsForAgents.spec.ts, beside this file) — but that one only reads
 * the i18n dictionary, and every leak found this time was in a `.vue`
 * template, where it could not see. This is its other half, and the two are
 * deliberately separate: that one knows what the copy should SAY, this one
 * only knows what it must never CONTAIN. Fixing the strings takes an hour
...
describe('the words on screen are for the people using the product', () => {
  it('no agent screen prints an internal rule, ADR or ticket id', () => {
