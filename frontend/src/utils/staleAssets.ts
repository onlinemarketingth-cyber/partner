/**
 * 2026-09-10 — reported on the Admin console ("ค้างไว้ ... กดปุ่มทำงานอะไรไม่ได้
 * ต้องกดปุ่ม refresh ถึงกลับมาทำงานได้") and true of this app for the same
 * reason. An agent's phone keeps this tab open for days; a deploy in between
 * is the normal case here, not the rare one.
 *
 * Kept in step with frontend-admin/src/utils/staleAssets.ts (ADR-003: the two
 * apps duplicate rather than share).
 *
 * ── THE TAB THAT WAS OPEN DURING A DEPLOY ──
 *
 * Every route in this app is a lazy `import()`, and Vite names each built
...
const RELOAD_MARK_KEY = 'portal.staleAssetReloadAt'
