...
beforeEach(() => {
  ...
  Object.defineProperty(window, 'location', {
    configurable: true,
    value: { assign, pathname: '/clients', search: '' },
  })
})
...
describe('recognising a chunk that is no longer on the server', () => {
  it('recognises what each browser actually says', () => {
    ...
    expect(isStaleAssetError(new TypeError('Failed to fetch dynamically imported module: https://x/assets/ClientsView-a1b2.js'))).toBe(true)
...
describe('reloading', () => {
  it('lands on the page the person was trying to open', async () => {
    ...
    handlers[0]?.(new TypeError('Failed to fetch dynamically imported module: /assets/x.js'), { fullPath: '/orders' })
  ...
  it('ignores a navigation that failed for any other reason', () => {
    ...
    handlers[0]?.(new Error('some component threw'), { fullPath: '/orders' })
  ...
  it('reloads once and then refuses, so a broken build cannot loop', () => {
     ...
     */
    expect(reloadForStaleAssets('/clients')).toBe(true)
    expect(reloadForStaleAssets('/clients')).toBe(false)
  ...
  it('reacts to a preload failure that never reached the router', () => {
    ...
    expect(assign).toHaveBeenCalledWith('/clients')
  })
})
