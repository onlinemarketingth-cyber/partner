...
const dayOptions = computed(() => {
  ...
  const month = localMonth.value ?? 1
...
function emitCurrent(): void {
  if (!localYearBE.value || !localMonth.value || !localDay.value) {
    lastEmitted = ''
  ...
  const safeDay = Math.min(localDay.value, daysInMonth(gregorianYear, localMonth.value))
  const datePart = `${gregorianYear}-${pad(localMonth.value)}-${pad(safeDay)}`
...
<template>
  <div class="flex gap-1.5 items-center flex-wrap">
    ...
    <input
      ...
      :value="localTime"
      :required="required"
      class="bg-surface-input text-ink-input px-2 py-2 rounded-lg border border-line-input text-sm"
      @change="onTimeChange"
    />
  </div>
</template>
