...
PLANS = [
    {
        ...
        "setup": [
            ...
            (S_OFF, "สายงาน (ใครเป็นหัวหน้าของใคร)",
             "ผู้ขาย → ชั้น 1 → ชั้น 2 → ชั้น 3 → ชั้น 4 (ชั้น 4 ไม่มีอัตรา จึงต้องไม่ได้เงิน)",
        ...
        "rows": [
            ...
            ("UAT หัวหน้าชั้น 4", "—", "—", 0.00, "ไม่ได้ตั้งอัตราชั้นที่ 4 ไว้ → ต้องไม่มีแถวในบัญชีเลย ไม่ใช่แถว ฿0"),
        ],
        "watch": [
            ...
            "ชั้น 4 มีตัวตนจริงและผ่าน Basic แล้ว แต่ต้องไม่มีแถวในบัญชีเลย เพราะไม่ได้ตั้งอัตราชั้นที่ 4 ไว้ "
            "— ถ้าเห็นแถว ฿0 หรือเห็นเขาได้อัตราของชั้นอื่น นั่นคือบั๊ก",
        ],
    },
    {
        "tab": "2 Binary",
        ...
        "setup": [
            ...
            *STEP4_TAIL,
            (S_OFF, "ขาซ้าย / ขาขวา ของแต่ละคน",
             "ขาซ้าย 1 คน (ขาย 2 ดีล) · ขาขวา 1 คน (ขาย 1 ดีล) ใต้ผู้สนับสนุน — ตั้งใจให้สองขาไม่เท่ากัน",
             "ตั้งที่เมนูจัดการผู้ใช้ระบบ — ตัวแทนที่ไม่ได้ตั้งขาจะไม่ส่งยอดขึ้นไปเลยและไม่มีคำเตือน"),
        ],
        "rows": [
            ("UAT ขาซ้าย", "direct", "10.00%", 2000.00, "ขาย 2 ดีล ได้ดีลละ ฿1,000 (2 แถวในบัญชี)"),
            ("UAT ขาขวา", "direct", "10.00%", 1000.00, "ขาย 1 ดีล"),
            ("UAT ผู้สนับสนุน", "binary_match", "10.00%", 1000.00,
             "หลังรันรอบจับคู่เท่านั้น: จับคู่ได้ min(ซ้าย ฿20,000, ขวา ฿10,000) = ฿10,000 → 10% "
             "· ถ้าได้ ฿2,000 แปลว่าระบบคิดจากขาที่มากกว่า ซึ่งผิด"),
        ],
        "watch": [
            ...
            "สองขาตั้งใจให้ไม่เท่ากัน (ซ้าย ฿20,000 · ขวา ฿10,000) — ถ้าเท่ากัน min() กับ max() จะให้คำตอบ"
            "เดียวกัน แล้วชุดทดสอบจะพิสูจน์กติกาของแผนนี้ไม่ได้เลย",
            "หลังรันรอบ ให้ดูหน้า “ประวัติรอบ Matching”: ต้องขึ้น ซ้าย ฿20,000 · ขวา ฿10,000 · Matched ฿10,000 "
            "· ยกยอด ฿10,000 — ช่องยกยอดคือที่เดียวที่บอกได้ว่าการยกยอดทำงานจริง",
        ],
    },
    {
        "tab": "3 Matrix",
        ...
        "setup": [
            ...
            ("ขั้นที่ 2 · ฟอร์ม “ค่าตั้งระดับบริษัทของแผน Matrix”", "ความลึก (Depth)", "2",
             "ตั้งใจให้น้อยกว่าจำนวนอัตราที่ตั้งไว้ข้างล่าง เพื่อให้ตรวจได้ว่าความลึกนี้บังคับจริง"),
            ...
            ("ขั้นที่ 2 · รายการ “อัตราค่าแนะนำตาม Level”", "Level 3", "Level 3 — 1.00%",
             "ตั้งไว้แต่ไม่มีวันได้จ่าย เพราะผังลึกแค่ 2 ชั้น — เป็นตัวล่อให้จับได้ว่าความลึกบังคับจริงหรือเปล่า"),
            (S_STEP3, "ค่าเริ่มต้นทั้งบริษัท", "10.00%",
        ...
        "rows": [
            ...
            ("UAT ชั้นเหนือสุด", "—", "—", 0.00,
             "อยู่ชั้นที่ 3 ของผัง · ตั้งอัตราชั้น 3 ไว้ 1% แต่ผังลึกแค่ 2 ชั้น → ต้องไม่มีแถว "
             "ถ้าได้ ฿70 แปลว่าระบบไม่สนความลึกที่ตั้งไว้"),
        ],
        "watch": [
            ...
            "มีอัตราชั้นที่ 3 ตั้งไว้ 1% ทั้งที่ผังลึก 2 ชั้น — ตั้งใจวางไว้เป็นกับดัก ชั้นเหนือสุดต้องได้ ฿0 "
            "ถ้าได้ ฿70 แปลว่าความลึกที่ตั้งไว้ไม่มีผลจริง",
        ],
    },
    {
        "tab": "4 Stairstep",
        ...
        "setup": [
            ...
            ("ขั้นที่ 2 · รายการ “บันไดอันดับ”", "อันดับที่ 2",
             "2. UAT ขั้นผู้นำ · ยอดขั้นต่ำ 50,000.00 · อัตรา 12.00%", ""),
            ("ขั้นที่ 2 · รายการ “บันไดอันดับ”", "อันดับที่ 3",
             "3. UAT ขั้นผู้จัดการ (Breakaway) · ยอดขั้นต่ำ 200,000.00 · อัตรา 20.00%",
             "ป้าย “(Breakaway)” สีเหลืองต้องขึ้นเฉพาะอันดับนี้"),
            ("ขั้นที่ 2 · รายการ “บันไดอันดับ”", "อันดับที่ 4",
             "4. UAT ขั้นผู้อำนวยการ · ยอดขั้นต่ำ 500,000.00 · อัตรา 25.00%",
             "มีไว้เพื่อให้ “ขั้นตัดสาย” มีอะไรให้หยุด — ขั้นนี้สูงกว่าผู้จัดการ ถ้าการเดินสายไม่หยุดจริง เขาจะได้ ฿500"),
            ...
            *STEP4_TAIL,
            (S_OFF, "อันดับปัจจุบันของแต่ละคน",
             "ผู้ขาย = เริ่มต้น · ผู้นำ = ผู้นำ · ผู้จัดการ = ผู้จัดการ (ตัดสาย) · ผู้อำนวยการ = ผู้อำนวยการ",
             "คำสั่ง seed วางอันดับให้โดยตรง ไม่ได้รอรอบคำนวณ — หน้าค่าแนะนำไม่มีช่องให้ดูหรือแก้อันดับรายคน"),
            (S_OFF, "สายงาน (ใครเป็นหัวหน้าของใคร)", "ผู้ขาย → ผู้นำ → ผู้จัดการ (ตัดสาย) → ผู้อำนวยการ",
             "ดูที่เมนูจัดการผู้ใช้ระบบ ช่อง “หัวหน้า”"),
        ],
        "rows": [
            ("UAT ผู้ขาย", "direct", "5.00%", 500.00, "5% ของ ฿10,000"),
            ("UAT ผู้นำ", "stairstep_override", "7.00%", 700.00, "ส่วนต่าง 12% − 5% = 7%"),
            ("UAT ผู้จัดการ (ตัดสาย)", "stairstep_override", "8.00%", 800.00, "ส่วนต่าง 20% − 12% = 8%"),
            ("UAT ผู้อำนวยการ", "—", "—", 0.00,
             "ขั้นสูงกว่าผู้จัดการ (25%) แต่ใต้เขาเป็นขั้นตัดสาย → การเดินสายหยุดตรงนั้น ต้องไม่มีแถว "
             "ถ้าได้ ฿500 แปลว่าระบบไม่สนขั้นตัดสาย"),
        ],
        "watch": [
            "สามแถวได้ไม่เท่ากันโดยตั้งใจ (500 / 700 / 800) — ถ้าเท่ากันหมด การสลับแถวจะมองไม่เห็น "
            "ชุดนี้เคยเป็น 500 ทั้งสามแถว แล้วพิสูจน์อะไรไม่ได้เลย",
            "ผู้อำนวยการอยู่เหนือผู้จัดการและมีขั้นสูงกว่า (25%) แต่ต้องได้ ฿0 เพราะใต้เขาเป็นขั้นตัดสาย "
            "— ถ้าได้ ฿500 แปลว่าระบบไม่ได้หยุดที่ขั้นตัดสายจริง",
    ...
    },
    {
        ...
        "setup": [
            ...
            ("ขั้นที่ 2 · ฟอร์ม “ค่าตั้งระดับบริษัทของแผน Generation”",
             ...
             "ฟอร์มนี้มีช่องเดียว · ตั้งใจให้น้อยกว่าจำนวนอัตราข้างล่าง เพื่อให้ตรวจได้ว่าค่านี้บังคับจริง"),
            ("ขั้นที่ 2 · รายการ “อัตราค่าแนะนำตาม Generation”",
             "Generation 1", "Generation 1 — 5.00%", ""),
            ("ขั้นที่ 2 · รายการ “อัตราค่าแนะนำตาม Generation”",
             "Generation 2", "Generation 2 — 3.00%", ""),
            ("ขั้นที่ 2 · รายการ “อัตราค่าแนะนำตาม Generation”",
             "Generation 3", "Generation 3 — 1.00%",
             "ตั้งไว้แต่ไม่มีวันได้จ่าย เพราะจำกัดไว้ 2 รุ่น — ตัวล่อแบบเดียวกับ Level 3 ของ Matrix"),
            (S_STEP3, "ค่าเริ่มต้นทั้งบริษัท", "10.00%", ""),
            *STEP4_HEAD,
            (S_STEP4, "4.3 อัตราหัวหน้าทีม · ค่าเริ่มต้นทั้งบริษัท", "ไม่มีรายการ",
            ...
            *STEP4_TAIL,
            (S_OFF, "บันไดอันดับ (ใช้ตัดสินว่าใคร “ตัดสาย”)",
             "UAT ขั้นเริ่มต้น 5% · UAT ขั้นผู้จัดการ 20% (Breakaway) — สร้างไว้ 2 ขั้นพอ เพราะแผนนี้ใช้แค่ว่า “ตัดสายหรือยัง”",
            ...
            (S_OFF, "อันดับปัจจุบันของแต่ละคน",
             "ข, ง และ จ = ขั้นผู้จัดการ (ตัดสาย) · ก, ค, ผู้ขาย = ขั้นเริ่มต้น",
             "นี่คือสิ่งที่ทำให้ ก และ ค ถูกข้าม — ดูขั้นที่ 2 ของชีตนี้"),
            (S_OFF, "สายงาน (ใครเป็นหัวหน้าของใคร)",
             "ผู้ขาย → ก → ข (ตัดสาย) → ค → ง (ตัดสาย) → จ (ตัดสาย)",
             "ดูที่เมนูจัดการผู้ใช้ระบบ ช่อง “หัวหน้า”"),
        ],
        "rows": [
            ("UAT ผู้ขาย", "direct", "10.00%", 1000.00, "10% ของ ฿10,000"),
            ...
            ("UAT หัวหน้า จ (ตัดสาย)", "—", "—", 0.00,
             "ตัดสายเหมือน ข และ ง ทุกอย่าง ต่างกันแค่เป็นรุ่นที่ 3 · ตั้งอัตรารุ่นที่ 3 ไว้ 1% "
             "แต่จำกัดไว้ 2 รุ่น → ต้องไม่มีแถว ถ้าได้ ฿100 แปลว่าระบบไม่สนจำนวนรุ่นสูงสุด"),
        ],
        "watch": [
            ...
            "จ ตัดสายแล้วเหมือน ข และ ง ทุกอย่าง ต่างกันแค่เป็นรุ่นที่ 3 และมีอัตรารุ่นที่ 3 ตั้งไว้ 1% "
            "— ต้องได้ ฿0 ถ้าได้ ฿100 แปลว่าจำนวนรุ่นสูงสุดไม่มีผลจริง",
    ...
    },
    {
        ...
        "setup": [
            ...
            (S_OFF, "สายงาน (ใครเป็นหัวหน้าของใคร)", "ผู้ขาย → ผู้แนะนำ → ผู้แนะนำชั้นบน",
             "ดูที่เมนูจัดการผู้ใช้ระบบ ช่อง “หัวหน้า”"),
        ],
        "rows": [
            ("UAT ผู้ขาย", "direct", "10.00%", 1000.00, "10% ของ ฿10,000 — ไม่ถูกหัก"),
            ("UAT ผู้แนะนำ", "override", "3.00%", 300.00, "3% ของ ฿10,000 บริษัทจ่ายเพิ่ม"),
            ("UAT ผู้แนะนำชั้นบน", "—", "—", 0.00,
             "ผ่าน Basic แล้วและอัตราเดียวกันใช้กับเขาได้ แต่แผนนี้จ่ายขึ้นชั้นเดียว → ต้องไม่มีแถว "
             "ถ้าได้ ฿300 แปลว่าระบบเดินสายแบบ Unilevel"),
        ],
        "watch": [
            "จบแค่ชั้นเดียว — ผู้แนะนำชั้นบนมีตัวตนจริง ผ่าน Basic แล้ว และอัตราเดียวกันใช้กับเขาได้ "
            "แต่ต้องได้ ฿0 · นี่คือสิ่งเดียวที่แยกแผนนี้ออกจาก Unilevel บนชุดทดสอบนี้",
...
def cover(wb, ranges):
    ...
    rows = [
        ...
        ("ทำไมตัวเลขถึงไม่กลม ๆ เท่ากัน",
         "ชุดนี้ออกแบบให้ “ผิดแล้วต้องเห็น” — ทุกแถวในแผนเดียวกันได้ไม่เท่ากัน และทุกแผนมีคนหนึ่งคนที่ "
         "ต้องได้ ฿0 เพราะกติกาสั่งให้หยุด ไม่ใช่เพราะสายงานหมดคนพอดี\n"
         "ชุดก่อนหน้านี้ตั้งสองขาของ Binary ให้เท่ากัน และตั้งส่วนต่างของ Stairstep ให้เท่ากันทั้งสามแถว "
         "ซึ่งแปลว่าต่อให้เครื่องคิดผิด ตัวเลขบนหน้าจอก็ยังเหมือนเดิม"),
        ("ค่าที่ใช้ทดสอบ",
         "ราคาสินค้า ฿10,000.00 · PV ฿7,000.00 · อัตราผู้ขาย 10% · หัวหน้า Unilevel 5/3/1% "
         "· ขั้น Stairstep 5/12/20/25% — เลือกให้คิดเลขในใจได้ ไม่ใช่ข้อเสนอว่าบริษัทจริงควรจ่ายเท่าไร (BR-7)"),
        ("ตั้งจ่ายเงินได้เลย",
