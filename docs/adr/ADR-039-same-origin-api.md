...
**สถานะ:** ✅ **เสร็จครบทั้ง 5 ขั้น 2026-08-21** · ยืนยันบน production แล้ว (ดู §10)
...
## 10. ✅ ผลจริงหลังจบขั้นที่ 5 (2026-08-21)

ยิงตรวจบน production จากเบราว์เซอร์ ทั้งสองโฮสต์:

```
admin.partner.syncvision.io   XSRF-TOKEN 1 ใบ · host-only ✅ · POST /login → 422
partner.syncvision.io         XSRF-TOKEN 1 ใบ · host-only ✅ · POST /login → 422
```

`isHostOnly: true` คือหลักฐานของทั้ง ADR นี้ — เซิร์ฟเวอร์เลิกออก cookie ผูกโดเมนแม่แล้ว
แต่ละโฮสต์ถือ cookie ของตัวเอง เบราว์เซอร์ไม่ส่งข้ามกันอีก

**`422` ไม่ใช่ error** — เป็น POST เปล่าที่ผ่าน CSRF แล้วไปติด validation ถ้า CSRF ยังพังจะได้ `419`
นี่คือวิธีพิสูจน์ CSRF โดยไม่ต้องใช้รหัสผ่านของใคร

ค่าที่ Laravel ใช้จริงหลังแก้:

```
$c['session']['domain'] → NULL
$c['session']['secure'] → bool(true)
```

### 10.1 สามอย่างที่เสียเวลาไปและควรจำไว้

**1. `cd` ที่โดนตัดตอน paste ทำให้ `sed` ไปรันผิดโฟลเดอร์**
คำสั่งลบ `SESSION_DOMAIN` รันใน `~` ซึ่งไม่มี `.env` แล้ว `||` ในบรรทัดตรวจสอบดันตอบว่า
"ไม่มีแล้ว ถูกต้อง" — **คำสั่งตรวจสอบโกหกเพราะตัวไฟล์ไม่มี ไม่ใช่เพราะค่าถูกลบ**
บทเรียน: คำสั่งที่แตะ production ต้องใช้ path เต็มเสมอ และตัวตรวจสอบต้องอ่านค่าที่แอปใช้จริง
(`bootstrap/cache/config.php`) ไม่ใช่แค่ `grep .env`

**2. `rsync` บน macOS ไม่รองรับ `--chmod`**
สคริปต์ตายก่อนอัปโหลด แต่ build/push สำเร็จไปแล้ว **deploy จึงดูเหมือนสำเร็จทั้งที่ไม่มีอะไรขึ้น**
แก้ที่ `deploy.sh` แล้ว: ตรวจความสามารถของ rsync แล้ว fallback ไป chmod ผ่าน SSH

**3. ag-lead สรุปผิดว่า "ตัวแก้ยังไม่ขึ้น"**
ตรวจแค่ chunk แรกที่ `index.html` อ้างถึง ทั้งที่ rolldown แยกโค้ดไปอีก chunk
บทเรียน: ตรวจ bundle ต้องไล่ทุก chunk ไม่ใช่ตัวแรก

### 10.2 งานต่อเนื่องที่เปิดออกมาจาก ADR นี้

- **`index.html` ต้องไม่ถูกแคช** — `Cache-Control: no-cache` กลับมาแล้วในทั้งสอง `.htaccess`
  (TASK-229 ถูกถอนออกตอน TASK-231 ซึ่งถอนผิดตัว ต้นเหตุจริงคือ mode 600)
- **`purgeDuplicateXsrfCookies()`** ใน `client.ts` ทั้งสองแอป — ล้าง cookie ค้างจากยุคโดเมนแม่
  ทำงานเฉพาะเมื่อเจอซ้ำจริง **ลบทิ้งได้เมื่อไม่มีผู้ใช้คนไหนถือ cookie จากก่อน 2026-08-21 แล้ว**
- **`__Host-` prefix** — ทำได้แล้วเพราะ `SESSION_DOMAIN` หายไป ดู §8
